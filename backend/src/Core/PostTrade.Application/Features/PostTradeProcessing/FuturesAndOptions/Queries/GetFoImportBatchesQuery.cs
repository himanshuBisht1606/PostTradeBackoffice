...
public record GetFoImportBatchesQuery(
    FoFileType? FileType,
    string? Exchange,
    ...
    int Page = 1,
    int PageSize = 20
) : IRequest<IEnumerable<FoImportBatchDto>>;

public class GetFoImportBatchesQueryHandler : IRequestHandler<GetFoImportBatchesQuery, IEnumerable<FoImportBatchDto>>
{
    private readonly IRepository<FoFileImportBatch> _batchRepo;
    private readonly ITenantContext _tenantContext;

    public GetFoImportBatchesQueryHandler(IRepository<FoFileImportBatch> batchRepo, ITenantContext tenantContext)
    {
        _batchRepo = batchRepo;
        _tenantContext = tenantContext;
    }

    public async Task<IEnumerable<FoImportBatchDto>> Handle(GetFoImportBatchesQuery request, CancellationToken cancellationToken)
    {
