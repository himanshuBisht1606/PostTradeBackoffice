...
public record ImportCmSttCommand(
    Stream FileStream,
...
public class ImportCmSttCommandHandler : IRequestHandler<ImportCmSttCommand, ImportResultDto>
{
    private readonly IRepository<CmFileImportBatch> _batchRepo;
    private readonly IRepository<CmStt> _sttRepo;
    ...
    public ImportCmSttCommandHandler(
        IRepository<CmFileImportBatch> batchRepo,
        IRepository<CmStt> sttRepo,
    ...
    public async Task<ImportResultDto> Handle(ImportCmSttCommand request, CancellationToken cancellationToken)
    {
        var tenantId = _tenantContext.GetCurrentTenantId();

        var existing = await _batchRepo.FirstOrDefaultAsync(b =>
            b.TenantId == tenantId &&
            b.FileType == CmFileType.Stt &&
        ...
        while ((line = await reader.ReadLineAsync(cancellationToken)) != null)
        {
            ...
            // Detect RptHdr section marker lines
            ...
            // Skip column header rows within each section
            ...
            try
            {
                ...
                var row = new CmStt
                {
                    ...
                    SttTaxAmt = fields.Length > 9 && decimal.TryParse(fields[9].Trim(), out var ta) ? ta : 0,
                    SttRate = fields.Length > 10 && decimal.TryParse(fields[10].Trim(), out var tr) ? tr : 0
                };
